...
class ControllerExtensionModuleSofiltershopby extends Controller {
 ...
 $price_0 = false; 
		...
		$data_template = array(
			...
			'text_price_0'			=> $this->language->get('text_price_0')
		);

		include_once(DIR_SYSTEM . 'library/template/Twig/Autoloader.php');
		Twig_Autoloader::register();
		if (file_exists(DIR_MODIFICATION.'catalog/view/theme/'.$this->config->get('theme_default_directory').'/template/soconfig/listing.twig')) {
			$loader = new Twig_Loader_Filesystem(DIR_MODIFICATION.'catalog/view/theme/'.$this->config->get('theme_default_directory') . '/template/soconfig/');
		}
		else {
